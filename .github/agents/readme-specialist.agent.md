...
- exclude the files `backend` and `frontend` in the root directory.
...
1. Use the `search` tool to locate documentation files.
2. Use the `read` tool to inspect existing files.
3. If improvements are needed, use the `edit` tool to modify the files directly.
4. After editing a file, present the changes and ask the user to confirm whether they want to keep them.

Do not only suggest changes in chat. Prefer editing the files using the `edit` tool whenever appropriate.

Only modify documentation files such as:
- README.md
- CONTRIBUTING.md
- other `.md` or `.txt` documentation files.

Never modify:
- source code files
- files inside `backend` or `frontend` directories in the root.