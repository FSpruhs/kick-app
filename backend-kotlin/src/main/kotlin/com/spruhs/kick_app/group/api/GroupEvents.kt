...
class GroupEventSerializer : Serializer {
        ...
        when (event.type) {
            GroupEvents.GROUP_CREATED_V1.name ->
                EventSourcingUtils.readValue(
                    event.data,
                    GroupCreatedEvent::class.java,
                )

            GroupEvents.GROUP_NAME_CHANGED_V1.name ->
                EventSourcingUtils.readValue(
                    event.data,
                    GroupNameChangedEvent::class.java,
                )

            GroupEvents.PLAYER_INVITED_V1.name ->
                EventSourcingUtils.readValue(
                    event.data,
                    PlayerInvitedEvent::class.java,
                )

            GroupEvents.PLAYER_ENTERED_GROUP_V1.name ->
                EventSourcingUtils.readValue(
                    event.data,
                    PlayerEnteredGroupEvent::class.java,
                )

            GroupEvents.PLAYER_REJECTED_GROUP_V1.name ->
                EventSourcingUtils.readValue(
                    event.data,
                    PlayerRejectedGroupEvent::class.java,
                )

            GroupEvents.PLAYER_PROMOTED_V1.name ->
                EventSourcingUtils.readValue(
                    event.data,
                    PlayerPromotedEvent::class.java,
                )

            GroupEvents.PLAYER_DOWNGRADED_V1.name ->
                EventSourcingUtils.readValue(
                    event.data,
                    PlayerDowngradedEvent::class.java,
                )

            GroupEvents.PLAYER_ACTIVATED_V1.name ->
                EventSourcingUtils.readValue(
                    event.data,
                    PlayerActivatedEvent::class.java,
                )

            GroupEvents.PLAYER_DEACTIVATED_V1.name ->
                EventSourcingUtils.readValue(
                    event.data,
                    PlayerDeactivatedEvent::class.java,
                )

            GroupEvents.PLAYER_REMOVED_V1.name ->
                EventSourcingUtils.readValue(
                    event.data,
                    PlayerRemovedEvent::class.java,
                )
    ...
    override fun aggregateTypeName(): String = "GroupAggregate"
}
