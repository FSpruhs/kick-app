...
class UserEventSerializer : Serializer {
    override fun serialize(
        ...
        return when (event) {
            is UserCreatedEvent ->
                Event(aggregate, UserEvents.USER_CREATED_V1.name, data, event.metadata)
            is UserNickNameChangedEvent ->
                Event(aggregate, UserEvents.USER_NICKNAME_CHANGED_V1.name, data, event.metadata)
            is UserImageUpdatedEvent ->
                Event(aggregate, UserEvents.USER_IMAGE_UPDATED_V1.name, data, event.metadata)
    ...
    override fun deserialize(event: Event): BaseEvent =
        when (event.type) {
            ...
            UserEvents.USER_NICKNAME_CHANGED_V1.name ->
                EventSourcingUtils.readValue(
                    event.data,
                    UserNickNameChangedEvent::class.java,
                )

            UserEvents.USER_IMAGE_UPDATED_V1.name ->
                EventSourcingUtils.readValue(
                    event.data,
                    UserImageUpdatedEvent::class.java,
                )

            else -> throw UnknownEventTypeException(event)
        }

    override fun aggregateTypeName(): String = "UserAggregate"
}
